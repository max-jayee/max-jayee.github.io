...
- 마이크로커널 아키텍처 스타일
  1. 플러그인(plug-in) 아키텍처라고도 한다.
  2. core system 과 plug-in componenet 라는 두 가지 아키텍처 요소로 구성된 비교적 단순한 모놀리식 아키텍처이다.
  3. core system: 시스템을 실행시키는 데 필요한 최소한의 기능을 정의한다.
  4. 컴파일 기반 플러그인 컴포넌트와 런타임 기반 플러그인 컴포넌트로 만들 수 있다.
  5. 디커플링을 위해 중앙 데이터베이스는 core system 만 접근한다.
  6. 일반적으로는 플러그인 레지스트리를 경유하여 사용하도록 구현한다.

- 서비스 기반 아키텍처 스타일
  1. 마이크로서비스 아키텍처 스타일의 일종으로, 아키텍처가 유연해서 가장 실용적인 아키텍처 스타일 중 하나이다.
  2. 프로토콜은 REST, messaging, RPC, SOAP 사용 가능하다.
  3. 데이터 베이스를 같이 사용할 수도, 별도로 사용할 수도 있다.
  4. 배포성, 내고장성, 모듈성, 전체 비용, 신뢰성, 시험성 이 높다.

- 이벤트 기반 아키텍처 스타일
  1. 확장성이 뛰어나고 고성능 어플리케이션 개발에 널리 쓰이는 비동기 분산 아키텍처 스타일이다.
  2. 어플리케이션은 대부분 request-based model 을 따른다. 이 모델은 어떤 액션을 수행하도록 시스템에 요청하면 request orchestrator 가 접수한다. 이 request orchestrator 는 request processor 에 확정적(deterministically)으로, 동기적(synchronously)으로 요청을 전달한다. request processor 는 요청을 받아 데이터베이스에서 정보를 조회/수정하는 등의 작업을 수행하는 식으로 요청을 처리한다.
  3. 주요 토폴로지는 mediator(중재자) topology 와 broker topology 가 있고, mediator 는 이벤트 처리 워크플로를 제어해야할 경우에, broker 는 신속한 응답과 동적인 이벤트 처리 제어가 필요할 때 사용된다.
  4. 브로커 토폴로지: 중앙에 이벤트 중재자가 없기에 rabbitmq, activemq, hornetq 등의 경량 메시지 브로커를 통해 브로드 캐스팅되는 식으로 이벤트 프로세서 컴포넌트에 분산되어 흘러간다.
  5. 중재자 토폴로지: 브로커 토폴로지의 단점을 일부 보완한다. 중재자가 이벤트를 선 받아 해당하는 프로세서의 큐에 넣어준다.
  6. 진화성, 내고장성, 모듈성, 성능, 확장성이 높다.

- 공간 기반 아키텍처 스타일
  1. tuple space 에서 공간 기반 아키텍처라는 명칭이 유래되었다.
  2. tuple space 는 공유 메모리를 통해 통신하는 다중 병렬 프로세서를 사용하는 기술이다.
  3. 메시징 그리드로는 HA 프록시, Nginx 로 구현한다.
  4. 데이터 그리드로는 redis 로 구현한다.
  5. 탄력성, 성능, 신뢰성, 확장성이 높다.

- 오케스트라 기반 서비스 지향 아키텍처 스타일
...
- 아키텍처 결정 기준
  ...
  7. 모놀리스냐 분산이냐
  8. 데이터를 어디에 둘 것인가?
  9. 서비스 간 통신은 동기, 비동기 중 어떤 스타일로 할 것인가?

- 안티 패턴
  1. 네 패를 먼저 보여주지마(Covering Your Assets) 패턴: 아키텍트가 잘못된 선택을 하는 것을 두려워 아키텍처 결정을 회피하거나 미루는 현상
  2. 무한반복 회의(Groundhog Day) 패턴: 사람들이 어떤 결정을 왜 했는지 모르고 회의만 계속 하는 것
  3. 이메일 기반 아키텍처(email-driven architecture) 패턴: 아키텍처 결정을 놓치거나 잊어버리고 그렇게 결정됐다는 사실조차 알지 못해 아키텍처 결정을 구현하지 못하는 상태
...
- 유능한 아키텍트
  1. 개발팀에 적절한 제약조건과 경계를 설정
  2. 팀원들이 서로 잘 협력할 수 있도록 독려하며 그들을 올바른 수준으로 가이드
  3. 팀이 기술을 보유하고 있는지 확인하고 걸림돌이 될 만한 요소를 제거
